    ...
    for parent,dirnames,filenames in os.walk(rootdir):    #三个参数：分别返回1.父目录 2.所有文件夹名字（不含路径） 3.所有文件名字
        for filename in filenames:                        
            ...
            if '.csv' in filename and judge ==1:
                ...
                file_path = rootdir + '/简单汇总.xls'
                style = xlwt.easyxf(num_format_str='#,##0.00')
                bk = xlrd.open_workbook(rootdir + '/简单汇总.xls') #获取表格中已有行数
...
root = Tk()
root.title('IVCurveDrawer v1.5.1')
Area = StringVar()
Width = StringVar()
Height = StringVar()
dark = StringVar()
...
Button(root, text = "简单统计IV曲线结果", command = simpleadd).grid(row = 0, column = 3,sticky=W)
